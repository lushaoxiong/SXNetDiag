...
//    NSString *icmpReplyType = [NSString stringWithFormat:@"%@", [LDSimplePing  icmpInPacket:packet]->sequenceNumber == 129 ? @"ICMPv6TypeEchoReply" : @"ICMPv4TypeEchoReply"];
//    NSString *successLog = [NSString
//                            stringWithFormat:@"%lu bytes from %@: icmp_seq=%u ttl=%@ time=%ldms",
//                         (unsigned long)[packet length], _hostAddress,
//                         sequenceNumber,
//                         icmpReplyType,
//                         [LDNetTimer computeDurationSince:_startTime] / 1000];
    NSString *successLog = [NSString
                            stringWithFormat:@"%lu bytes from %@: icmp_seq=%u time=%ldms",(unsigned long)[packet length], _hostAddress,
                            sequenceNumber,
                            [LDNetTimer computeDurationSince:_startTime] / 1000];
    //记录ping成功的数据
    if (self.delegate && [self.delegate respondsToSelector:@selector(appendPingLog:)]) {
